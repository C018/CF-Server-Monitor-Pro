      ...
      if (idParam && !isAjax) {
        ...
        <body class="${sys.theme || 'theme1'}">
          ...
          <script>
            ...
            // ===== 历史图表 X 轴时间刻度：目标刻度数随 chart 宽度自适应 + 步长随数据跨度自适应（整点对齐，北京时间 UTC+8）=====
            const HIST_STEP_HOURS = [0.5, 1, 2, 3, 4, 6, 8, 12, 24, 48, 72, 168];
            const HIST_SAMPLE_MS = 5 * 60 * 1000;   // 无采样时间戳时的兜底采样间隔（5 分钟）
            const HIST_BJ_OFFSET = 8 * 3600 * 1000; // 北京时间 UTC+8 偏移
            const HOUR_MS = 3600000;
            // 目标刻度数：宽度 <420px -> 3；<700px -> 4；<980px -> 5；否则 6（resize 时自动重算）
            ...
            // 数据跨度：优先取该图表 __ts 首尾差；无 __ts 时按数据点数 × 5 分钟兜底
            function histSpanMs(chart) {
               const ts = (chart && chart.data) ? chart.data.__ts : null;
               if (Array.isArray(ts) && ts.length >= 2) {
                  const a = ts[0], b = ts[ts.length - 1];
                  if (typeof a === 'number' && typeof b === 'number' && isFinite(a) && isFinite(b) && b > a) return b - a;
               }
               const n = (chart && chart.data && chart.data.labels) ? chart.data.labels.length : 0;
               return n >= 2 ? (n - 1) * HIST_SAMPLE_MS : 0;
            }
            // 步长自适应：候选步长中取满足「跨度 / 步长 <= 目标刻度数」的最小值
            function histStepMsOf(chart) {
               const span = histSpanMs(chart);
               if (!(span > 0)) return HOUR_MS;
               const need = span / histTargetTicks(chart);
               for (let i = 0; i < HIST_STEP_HOURS.length; i++) {
                  const ms = HIST_STEP_HOURS[i] * HOUR_MS;
                  if (ms >= need) return ms;
               }
               return HIST_STEP_HOURS[HIST_STEP_HOURS.length - 1] * HOUR_MS;
            }
            // 解析历史标签（新格式 "MM-DD HH:mm" / 旧脏数据 "HH:MM"）为可比较的伪时间戳；失败返回 null
            function histLabelMs(lbl) {
               if (typeof lbl !== 'string') return null;
               const parts = lbl.split(' ');
               let dayPart = null, timePart = null;
               if (parts.length === 2 && parts[0].indexOf('-') > 0) { dayPart = parts[0]; timePart = parts[1]; }
               else if (parts.length === 1 && parts[0].indexOf(':') > 0) { timePart = parts[0]; }
               else return null;
               const hm = timePart.split(':');
               if (hm.length < 2) return null;
               const hh = parseInt(hm[0], 10), mi = parseInt(hm[1], 10);
               if (!isFinite(hh) || !isFinite(mi)) return null;
               let mo = 0, dd = 0;
               if (dayPart !== null) {
                  const dm = dayPart.split('-');
                  if (dm.length !== 2) return null;
                  mo = parseInt(dm[0], 10); dd = parseInt(dm[1], 10);
                  if (!isFinite(mo) || !isFinite(dd)) return null;
               }
               return ((mo * 31 + dd) * 24 + hh) * HOUR_MS + mi * 60000;
            }
            // 第 index 个采样点的毫秒时间戳：优先 __ts，缺失时回退标签解析
            function histSampleMs(chart, index) {
               const ts = (chart && chart.data) ? chart.data.__ts : null;
               ...
               return histLabelMs(labels[index]);
            }
            // 该图表是否具备与 labels 等长的真实采样时间戳
            function histHasTs(chart) {
               const labels = (chart && chart.data && chart.data.labels) || [];
               const ts = (chart && chart.data) ? chart.data.__ts : null;
               return Array.isArray(ts) && labels.length > 0 && ts.length === labels.length;
            }
            // 刻度文本（北京时间 UTC+8）：步长 >= 24h 或落在 00:00 整点 -> MM-DD，其余 -> HH:mm
            function histTickLabel(ms, stepMs, realTs) {
               const p = (n) => String(n).padStart(2, '0');
               if (realTs) {
                  const d = new Date(ms + HIST_BJ_OFFSET);
                  const mo = p(d.getUTCMonth() + 1), dd = p(d.getUTCDate());
                  const hh = d.getUTCHours(), mi = d.getUTCMinutes();
                  if (stepMs >= 24 * HOUR_MS) return mo + '-' + dd;
                  if (hh === 0 && mi === 0) return mo + '-' + dd;
                  return p(hh) + ':' + p(mi);
               }
               const totalMin = Math.floor(ms / 60000);
               const hh = Math.floor(totalMin / 60) % 24, mi = totalMin % 60;
               if (stepMs >= 24 * HOUR_MS || (hh === 0 && mi === 0)) {
                  const days = Math.floor(totalMin / 1440);
                  const mo = Math.floor(days / 31), dd = days - mo * 31;
                  return p(mo) + '-' + p(dd);
               }
               return p(hh) + ':' + p(mi);
            }
            ...
            // 稀疏刻度过滤：仅保留「跨步长边界」的采样点（bucket = Math.floor(ts / stepMs) 与前一采样点不同），竖网格线随之对齐
            function histAfterBuildTicks(axis) {
               const chart = axis && axis.chart;
               const ticks = (axis && axis.ticks) || [];
               if (!chart || !ticks.length) return ticks;
               const stepMs = histStepMsOf(chart);
               const out = [];
               let prevBucket = null;
               for (let i = 0; i < ticks.length; i++) {
                  const tk = ticks[i];
                  const ms = histSampleMs(chart, histTickIndex(chart, tk ? tk.value : null, i));
                  if (ms === null) { out.push(tk); continue; }
                  const bucket = Math.floor(ms / stepMs);
            ...
            function histTicks(fontColor) {
               ...
               return {
                  ...
                  callback: function(value, index) {
                     ...
                     const ms = histSampleMs(chart, idx);
                     if (ms === null) {
                        const labels = (chart.data && chart.data.labels) || [];
                        const lb = labels[idx];
                        return (lb === undefined || lb === null) ? '' : String(lb);
                     }
                     const stepMs = histStepMsOf(chart);
                     return histTickLabel(Math.floor(ms / stepMs) * stepMs, stepMs, histHasTs(chart));
                  }
               };
            }
